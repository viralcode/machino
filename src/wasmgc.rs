...
//! the default backend: extern fns beyond print are not provided by the
//! reference GC host, and spawn/join require the default backend (E072).
//!
...
// checked int arithmetic: trap (unreachable) on overflow, like the
// linear-memory backend
const HELP_IADD: u32 = N_IMPORTS + 8; // (i64, i64) -> i64
const HELP_ISUB: u32 = N_IMPORTS + 9; // (i64, i64) -> i64
const HELP_IMUL: u32 = N_IMPORTS + 10; // (i64, i64) -> i64
const HELP_IREM: u32 = N_IMPORTS + 11; // (i64, i64) -> i64
const HELP_IDIV: u32 = N_IMPORTS + 12; // (i64, i64) -> i64
const HELP_LEN_CP: u32 = N_IMPORTS + 13; // (bytes) -> i64
const HELP_CHAR_AT_CP: u32 = N_IMPORTS + 14; // (bytes, i64) -> i64
const HELP_SUBSTR_CP: u32 = N_IMPORTS + 15; // (bytes, i64, i64) -> bytes
const HELP_CHR_CP: u32 = N_IMPORTS + 16; // (i64) -> bytes
const HELP_HASH_STR: u32 = N_IMPORTS + 17; // (bytes) -> i64
    ...
    fn expr_type_raw(&self, expr: &Expr, ctx: &mut Ctx) -> Result<Type, Diagnostic> {
        Ok(match &expr.kind {
            ...
            ExprKind::Call(name, args) => match name.as_str() {
                "print" => Type::Unit,
                "len" | "char_at" | "to_int" | "len_cp" | "char_at_cp" | "hash" => Type::Int,
                "to_float" => Type::Float,
                "substr" | "chr" | "substr_cp" | "chr_cp" => Type::Str,
        ...
        match &expr.kind {
            ...
            ExprKind::Call(name, args) => match name.as_str() {
                ...
                "len_cp" => {
                    self.compile_expr(code, &args[0], ctx, None)?;
                    code.push(OP_CALL);
                    uleb(code, HELP_LEN_CP as u64);
                }
                "char_at_cp" => {
                    self.compile_expr(code, &args[0], ctx, None)?;
                    self.compile_expr(code, &args[1], ctx, None)?;
                    code.push(OP_CALL);
                    uleb(code, HELP_CHAR_AT_CP as u64);
                }
                "substr_cp" => {
                    self.compile_expr(code, &args[0], ctx, None)?;
                    self.compile_expr(code, &args[1], ctx, None)?;
                    self.compile_expr(code, &args[2], ctx, None)?;
                    code.push(OP_CALL);
                    uleb(code, HELP_SUBSTR_CP as u64);
                }
                "chr_cp" => {
                    self.compile_expr(code, &args[0], ctx, None)?;
                    code.push(OP_CALL);
                    uleb(code, HELP_CHR_CP as u64);
                }
                "hash" => {
                    ...
                    match ty {
                        ...
                        Type::Str => {
                            ...
                            code.push(OP_CALL);
                            uleb(code, HELP_HASH_STR as u64);
                        }
