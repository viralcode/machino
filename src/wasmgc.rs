...
impl Compiler {
        ...
        match &expr.kind {
            ...
            ExprKind::Bin(op, lhs, rhs) => {
                ...
                let opcode = match (&lt, op) {
                    // i64.div_s traps natively on /0 and i64::MIN / -1
    ...
    fn body_iadd(&self) -> Vec<u8> {
        ...
        c.extend_from_slice(&[OP_IF, 0x40, OP_UNREACHABLE, OP_END]);
        c.extend_from_slice(&[OP_LOCAL_GET, 2, OP_RETURN, OP_END]);
        b.extend_from_slice(&c);
        b
    }

    /// isub(a, b) -> a - b, trapping on overflow: ((a^b) & (a^c)) < 0.
    /// locals: 2 = c (i64)
    fn body_isub(&self) -> Vec<u8> {
        let mut b = Vec::new();
        b.extend_from_slice(&[1, 1, I64]);
        let mut c = Vec::new();
    ...
    /// imul(a, b) -> a * b, trapping on overflow (division-based check;
    /// a == 0 and a == -1 special-cased). locals: 2 = c (i64)
    fn body_imul(&self) -> Vec<u8> {
        let mut b = Vec::new();
        b.extend_from_slice(&[1, 1, I64]);
        let mut c = Vec::new();
        // if a == 0 return 0
        ...
        // if a == -1: b == i64::MIN traps, else return 0 - b
        ...
        c.push(OP_I64_CONST);
        sleb(&mut c, i64::MIN);
        c.extend_from_slice(&[OP_I64_EQ, OP_IF, 0x40, OP_UNREACHABLE, OP_END]);
    ...
    /// irem(a, b) -> a % b. Native i64.rem_s traps on b == 0 but *returns 0*
    /// for i64::MIN % -1; machino treats that as overflow, so trap explicitly.
    fn body_irem(&self) -> Vec<u8> {
        let mut b = Vec::new();
        ...
        c.extend_from_slice(&[0x71 /* i32.and */, OP_IF, 0x40, OP_UNREACHABLE, OP_END]);
